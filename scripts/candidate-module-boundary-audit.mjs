...
for (const file of walk(APP).sort()) {
  ...
  const transactionBoundaries = [...functions.values()].flatMap((record) => {
    if(record.name==='runInstalledMaintenance' && isOrdered(record.body,['platform-installed-runtime','installedClient(', '--install-id', 'authority.homeRealPath', 'Foundation Maintenance', 'privateDirectory(', 'followMaintenance(']))return [{function:record.name,kind:'verified-installed-identity-owned-maintenance-records-and-exact-manager-continuation'}];

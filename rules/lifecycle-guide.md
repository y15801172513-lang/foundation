...
分别报告程序、可选 Codex Skill、项目接入。启动器返回 FOUNDATION_ACQUISITION_PROGRESS_PAGE 或 FOUNDATION_SINGLE_PAGE 时，在 Codex 内置浏览器打开该 URL，并继续等待同一工具句柄；目录、程序和 Skill 确认都在原页进行。下载与校验在可信 npm 中执行，选择意向不是安装批准。页面显示真实阶段与字节，未知总量不显示百分比。npm 自身启动之前的下载只能由宿主说明，临时页面不是永久地址。只获取材料的 --acquire 页面仍为只读，不启动安装。
...
对话直接消费同次终态与页面投影；程序成功而可选 Skill 失败分别说明，注册文件不等于新任务已识别。只读查询无新状态时保留上次已核实结果。卸载后的结果使用既有独立回执；不为查询保留完整程序或常驻服务。
...
更新成功后从新稳定入口处理后续操作。细节见公共 `docs/cache-cleanup.md`；新实现不代表用户已更新，不修改旧安装补接口。

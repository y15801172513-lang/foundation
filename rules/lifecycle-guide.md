...
页面总览将尚需处理的动作和已经核实的结果分开。获取、目录选择、程序变更与健康检查持续关联同次流程；选择页的对话能力选择只是意向，不是注册批准。若用户选择本次跳过，不重复询问；若选择继续，程序健康后按当前规则分别请求材料准备与 Codex 注册计划，并关联前一次实际 session/plan。材料准备完成尚不能去新任务使用；注册完成仅证明文件已就位，新任务发现仍另验。未作选择时说明“程序完成，对话能力可选待选择”，项目接入不阻止软件收尾。

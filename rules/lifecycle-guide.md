...
本载荷的接续需要已核验的 summon 0.1.5 或后续兼容版本；源码版本不代表 npm 已发布，执行前按公共安装说明核验实际可获取版本。旧 0.1.4 的程序结束不证明所选 Skill 已完成。

默认首装选中 Codex 后，由启动器在程序健康通过后自动准备材料及注册的精确确认；已有效的材料复用。保持同一工具句柄等待，打开每次返回的真实 loopback URL，不另发 capability 命令。用户拒绝或取消时程序结果保留，明确 Skill 待续，不重复询问。

用户明确要求更新时，核验当前稳定入口、目标发行及任务权限后使用 `summon foundation --update --root <已核验安装根> --version <固定目标版本>`；卸载使用 `summon foundation --uninstall --root <已核验安装根>`。更新保留已注册副本，由新 current 准备完整归属的精确刷新；卸载才先准备解除确认并接续程序移除。每项仍需本人确认。无 Skill 不新增；解除拒绝/失败不继续程序删除。0.2.10 的第一次更新由旧稳定入口执行，确认页仍是旧界面；更新后重新解析新 current，才进入新版材料/注册流程。用户修改或未知 Skill 保留，不能承诺刷新成功。
...
终态以整个所选流程的 npm 记录为准，单个 manager 结果只是子步骤。卸载完成后的独立回执与缓存结果保留；不能调用已删除的 launcher，也不能以其消失判定成功。下面的底层计划说明用于独立操作与诊断，不替代上述接续入口。

## 同一次流程的结果

分别报告程序、可选 Codex 对话能力、项目接入。获取启动器返回 FOUNDATION_ACQUISITION_PROGRESS_PAGE 时，在 Codex 内置浏览器打开该只读 URL，并继续等待同一工具句柄；下载与校验在可信 npm 中执行，尚未批准安装。页面显示真实阶段与字节，未知总量不显示百分比。npm 自身启动之前的下载只能由宿主说明，临时页面不是永久地址。选择/确认页 URL 来自校验后的运行程序，不能从外部网页拼接。

后续 manager request-plan 可附 --previous-plan-ref <上一步返回的 planRef>，或在首装之后附 --previous-session-id <首次安装 sessionId>，关联同一安装的真实步骤；两者互斥。参数位于操作参数之外，只关联结果，不复制或扩大前次批准；安装、Skill 注册/刷新/移除、项目接入各自仍需精确确认。记录缺失或安装不匹配保持待核实，不靠浏览器记忆或调用方成功布尔值补证。中止当前步骤不撤销已完成步骤。

对话直接消费同次终态与页面投影；程序成功而可选 Skill 失败分别说明，注册文件不等于新任务已识别。只读查询无新状态时保留上次已核实结果。卸载后的结果使用既有独立回执；不为查询保留完整程序或常驻服务。旧版更新确认仍由旧 manager 展示，更新后从稳定入口重新解析 current 才拥有新版行为。

页面总览将尚需处理的动作和已经核实的结果分开。获取、目录选择、程序变更与健康检查持续关联同次流程；选择页的对话能力选择只是意向，不是注册批准。若用户选择本次跳过，不重复询问；若选择继续，由启动器在程序健康后分别准备材料与 Codex 注册计划，并关联前一次实际 session/plan；对话只观察产品接续，不额外发起重复计划。材料准备完成尚不能去新任务使用；注册完成仅证明文件已就位，新任务发现仍另验。未作选择时说明“程序完成，对话能力可选待选择”，项目接入不阻止软件收尾。

每一页从精确计划读取操作对象：注册、刷新和移除的主要位置是实际 Codex 用户级 Skill 目录；Foundation 程序目录是关联程序。项目、获取材料和回执分别命名，缺失位置保持待核实。旧记录缺失、安装身份变化或当前步骤失败时，保留前面已核实结果，不用当前失败覆盖全部流程，也不重放先前批准。流程关联标识只有展示用途，不赋予执行权限。
...
检查状态用 installed `manager inspect`；检查能力从当前 descriptor 的 endpoint 定位 manifest，不固定版本。更新只准备当前支持的计划，接入仍需独立 enable。当前 037 Runtime 支持完整归属的 Skill 刷新：更新后先核对当前 capability receipt；旧内容身份不匹配时独立 capability-install，然后以 connectCodex: true 请求独立 capability-register。同一安装、原两文件及归属回执完整匹配时，计划披露精确替换而保留未知文件；已修改、缺失或他人归属则停止。不要默认 capability-uninstall 后重装，不因旧 Skill 残留声称 current 已接入。旧版尚无此能力时按其真实拒绝结果处理，不自动解除。文件存在不证明新任务发现。
...
安装/更新成功后先读取既有 Skill 选择和产品结果。已选中时继续产品返回的确认；已跳过或拒绝时不重复询问。只有尚未选择且用户希望接入时才说明可选能力。文件、注册与新任务发现分开，不覆盖用户修改。

...
新安装由当前用户共用一份程序与一份用户级 Skill，不询问业务项目路径，不自动接入所有项目。用户说“用 Foundation 做这个项目”时，核验明确的当前项目与已有绑定；未接入则按现有独立确认接入、准备、采用，已就绪则继续使用而非重新初始化。多个项目可以共用程序，但每个项目的文件、事实、规则和例外各自保管；不能把项目 A 的批准用于项目 B。发现已有安装或同名 Skill 冲突时先核实并使用、更新现有安装，不自动创建第二份、迁移或覆盖。

...
export function assetReuseSummary(item, facts = {}) {
  ...
  return {state, taskId: assessment?.taskId || null, scopeRevision: assessment?.scopeRevision ?? null,
    label: registered.length ? `${registrationLabel} · ${definitionVerified ? '定义已静态核实' : '定义静态核实待核'}` : registrationLabel,
...
export function projectAssets(facts = {}) {
  ...
  const make = (item, assetType) => {
    ...
    return {...item, deliveryAssessment:facts.delivery?.assessment || null, assetModelState:item.assetModel?'declared-not-runtime-verified':'legacy-unreviewed', contentFact: item, contentIssues: integrity.issues.filter(issue => issue.objectId === id), assetId: id, id, assetType, name: item.name || item.label || id, status: item.status ?? null, source: item.source ?? null, implementationPath: item.implementationMapping || null, previewRoute: item.previewRoute || null, family: item.family || null, variant: item.variant || null, variants: array(item.variants), responsibility: item.assetModel?.responsibility || item.responsibility || item.description || item.summary || null, composes: array(item.composes), dependencies: array(item.dependencies), usageLocations, usedByPages, usedByComponents: array(item.usedByComponents), reuseSummary, reuseDecision: reuseSummary.label, verificationStatus: item.verificationStatus ?? null, value: item.value ?? null, references: references(item.references || item.aliases), tokenScope: item.scope ?? null, recentChanges: relevant, impactPages, impactComponents, figma: mapping ? {id: mapping.id, nodeId: mapping.nodeId, nodeType: mapping.nodeType, verificationStatus: mapping.verificationStatus} : {status: '未映射'}, conflicts: array(item.conflicts), missing: [...array(item.missing), ...integrity.issues.filter(issue => issue.objectId === id).map(issue => issue.message)], pending: array(item.pending), relations: relations.filter((relation) => usedByPages.some((usage) => relation.from === usage.pageId || relation.to === usage.pageId))};
  };

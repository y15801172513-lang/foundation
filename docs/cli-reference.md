...
npm 0.1.12 在真实获取前返回唯一单页 URL，仍需 Codex 打开并等待。同页完成目录/Skill 意向、下载核验、程序确认、Skill 准备及宿主注册的独立确认，结果保留已经核实的成功；计划关联不继承任何批准。`summon foundation --version 0.2.17` 固定当前试用版。旧多页产品线退出支持，不回退旧页面；下面 036 段仅记录历史接口，不是当前推荐入口。

...
本版加强真实组件复用、任务范围交付核验与工作台状态刷新。组件定义、运行行为、布局证据与真人接受分别记录，跨标签页结果及关系详情保持同步。

当前健康检查（沿用 0.2.15 修正）：`--foundation-health` 只报告当前载荷的版本与运行时，不派生安装/维护权限，不写入安装状态。调用方仍先核验载荷完整性，再严格检查唯一健康结果；它不代表已经安装成功。

安装执行退出契约：已核实完成为 `0`；失败、部分完成、结果待核实或服务中断为 `1`；明确取消为 `2`；确认到期为 `3`。启动器以本次产品结果为准，正常关闭确认服务不代表成功。终态结果文件记录对应退出码。`--inspect` / `--status` 属于只读查询，查询命令成功不等于被查询安装成功。回滚完成只表示恢复处理完成，原安装仍失败；健康检查错误、退出码、信号、超时和执行阶段保留在折叠技术详情与同次记录中。
...
npm 0.1.10 在真实获取前返回唯一单页 URL，仍需 Codex 打开并等待。同页完成目录/Skill 意向、下载核验、程序确认、Skill 准备及宿主注册的独立确认，结果保留已经核实的成功；计划关联不继承任何批准。`summon foundation --version 0.2.16` 固定当前试用版。旧多页产品线退出支持，不回退旧页面；下面 036 段仅记录历史接口，不是当前推荐入口。

...
040 单页体系之前的正式产品线（已发布 0.2.11 及更早运行版、配套旧 npm 入口）退出维护，不再承担安装支持、升级迁移或兼容修复。旧版仅保留历史身份，不作为当前推荐入口或新版验收目标。退出维护不等于授权删除：真实安装、项目、Skill、缓存、Git 历史和远端发行全部保留。

新版只支持声明受控单页接续能力的程序与入口。发现旧安装时安全停止，说明需另行确认卸载/重新安装；不自动删除、不回退旧页面或旧发行。工程候选暂沿用的版本字段不能代替正式发行身份，亦不能据此当作旧版或上传。

当前单页试用版为程序 0.2.14 / npm 0.1.8，推荐入口见[安装说明](install-with-codex.md)。线上旧版本继续保留历史身份；没有改写旧 Release、执行 npm deprecate 或操作用户旧安装。发行和真人验收是独立状态。

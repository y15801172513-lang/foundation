...
使用时先核验实际 installed current 的帮助与支持列表，不猜测版本。本地 038 候选的持续交付检查尚待发布；旧安装不会因源码修改自动获得新接口。

明确使用 Foundation 后，相关制作继续同一工具选择；打开页面不自动授权项目接入。业务根不明确时先选根，再按当前安装的准备/采用计划单独确认。项目规则已就绪即可继续，不再次问是否使用 Foundation。关闭工作台不等于停用。

源码完成不等于交付完成：受影响事实批次、用户要求的工作台预览、只读交付检查与重开各自核验。取消/失败则保留代码，说明“代码完成，Foundation同步待完成”。当前候选记录源码摘要；旧记录缺摘要保持待核实，不自动补写。详细流程取随包 `foundation-making.md`，不复制另一套规则。

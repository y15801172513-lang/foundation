...
单页试用版 [0.2.15](https://github.com/y15801172513-lang/foundation/releases/tag/v0.2.15) 与 [npm 0.1.9](https://www.npmjs.com/package/@josephyulei/summon-foundation/v/0.1.9) 已发布。旧正式产品线已退出维护；唯一支持政策见[安装与生命周期合同](install-platform-024.md#现行支持策略)，当前命令见[安装说明](install-with-codex.md)。

## 当前发行：0.2.15（2026-09-17）

运行载荷绑定公共源码 `eb98b36785d82e834559bcfce5cad10203798a90`，对应私有源码 `07bb192c0f1d188e578e356f3937cbdc2ce0d491`；npm 0.1.9、随包 Skill 0.2.5。当前用户共用一份程序与可选对话能力，安装不选择业务项目；项目以后独立接入。确认、未知文件保护和独立卸载回执保留。文档提交不改变运行载荷身份。

双次干净重建、公开导出、最终归档及发行资产证明已核验；本轮最终载荷的无源码祖先首装、Skill 接续、更新重开、卸载保留，以及健康失败后的诊断、回滚和非零退出均通过隔离工程验证。未改的项目制作功能沿用历史证据，不冒称本轮重新验收。npm registry 实际包已回读匹配。未恢复此前叫停的长时间公网安装演练；真人安装、更新、卸载、新任务 Skill 发现与主动告知仍待独立验收。

## 历史发行：0.2.13（2026-09-16）
...
最终载荷的完整入口组合、选装/跳过 Skill、更新刷新与暂存清理、卸载保留及修改文件保护通过隔离工程验证。旧版不迁移或删除。参考图像一比一对照与部分历史浏览器观察仍未完成，本版作为试用候选，不将这些缺口写成通过。本人安装、Skill 新任务发现和无追问主动告知仍需独立验收；发布不修改用户安装。下文为历史记录。

## 历史发行：0.2.11（2026-09-15）

公共不可变 [v0.2.11](https://github.com/y15801172513-lang/foundation/releases/tag/v0.2.11) 对应公共源码 `ede0dd48296f7498f77d733996c663f55663721c`、私有源码 `2f247a333a2f42f5b640f98469b5058b6eea8761`。文档提交不改变运行载荷身份。配套 npm 0.1.5 让所选 Skill、更新/卸载与恢复由程序接续，保留独立确认与用户数据保护；真人安装、新对话发现和主动告知仍需独立验收。入口见[安装说明](install-with-codex.md)。

## 历史发行：0.2.10（2026-09-14）

公共不可变 [v0.2.10](https://github.com/y15801172513-lang/foundation/releases/tag/v0.2.10) 已发布，三项资产和 GitHub 证明已核验。运行载荷绑定公共源码 `163972c56c94f8b89f09e72411d28c8540dc4f0c`，对应私有源码 `c2a0d16dfb2e753ae93f3476d797fbb1075a13ea`，两库历史独立。文档提交不改变运行载荷身份。

配套薄入口为 [npm 0.1.4](https://www.npmjs.com/package/@josephyulei/summon-foundation/v/0.1.4)，新增获取只读进度页；生命周期展示当前对象、剩余必要动作与实际终态，不把材料准备等同宿主发现。项目制作、事实同步与只读 delivery-check 连贯使用原工作台。入口与限制见[安装说明](install-with-codex.md)。

干净公开源码直接组合 46/46、最终载荷入口组合 2/2、正式 SHA 获取工程组合 2/2 通过；实际载荷的 0.2.9 升级、规则/Skill 刷新、项目制作重开、回退和卸载保留通过。网络/证明替身的工程获取与线上真实匿名验证是独立记录；本人安装、Skill 新任务发现和主动告知仍 pending。新版发布不自动修改用户安装。

## 历史发行：0.2.9（2026-09-14）

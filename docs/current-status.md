...
单页试用版 [0.2.31](https://github.com/y15801172513-lang/foundation/releases/tag/v0.2.31) 与 [npm 0.1.13](https://www.npmjs.com/package/@josephyulei/summon-foundation/v/0.1.13) 已发布。旧正式产品线已退出维护；唯一支持政策见[安装与生命周期合同](install-platform-024.md#现行支持策略)，当前命令见[安装说明](install-with-codex.md)。

## 当前发行：0.2.31 / npm 0.1.13

新增验证调用统一经过精确宿主入口，公开模块边界契约通过；同时包含重复组件与 SVG 的源码对象定位、复制失败恢复、当前证据绑定的资产准入及独立预览。运行载荷绑定公共源码 `9aaaf3c28498df9301cfb5993b77fe364d66b583`，来自私有源码 `70015e7ac09cdd6ee48465017aab3c3a797bcc48`；后续文档提交不改变运行载荷身份。npm 启动器未变，复用 0.1.13。

最终载荷六组发布检查通过，包括 17 项更新恢复回归，无失败或跳过；另验证 0.2.19 原位升级保留项目、80 项实际浏览器对象选择与源码解析、字号 24→28 的三视口传播、复制权限拒绝与重试，以及五维交付和双页签资产失效恢复。不可变发行、全部资产证明及匿名下载字节已核对；匿名下载并完整核验的实际发行包在隔离账户完成新装与重开；首次默认 npm 在线试装曾因慢速下载超时，未记作通过。该工程确认不证明真实用户安装、真人接受或新 Codex 任务发现。

## 历史发行：0.2.19 / npm 0.1.13
...
最终公开载荷通过五组生命周期验收（直接测试 43/43、入口 13/13、获取夹具 2/2、隔离安装更新卸载、健康失败回滚），以及四项真实复用任务和 14×2 标签页状态/详情恢复浏览器验收。发行证明、三个资产摘要及 npm 实际包已回读；匿名入口脚本与目录清单匹配。工程确认不等于真人接受；真实安装、更新、卸载、新任务 Skill 发现与主动告知仍需独立验收。

## 历史发行：0.2.15（2026-09-17）

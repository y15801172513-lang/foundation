...
035 修正版（npm 0.1.2 / 运行版 0.2.7）已发布并回读核验发行字节。以下入口可用于新对话试用；真人安装、接续与主动回复仍独立验收：
...
旧 npm 0.1.1 / v0.2.6 只支持先由对话取得目录再 `--prepare` 的流程；裸命令止于 `RELEASE_DISCOVERED_NOT_ACQUIRED` 是旧版已知缺陷。新试用使用上面的 0.1.2 入口，不复用旧获取缓存或旧计划。现有安装不会因为新版发布自动更新。

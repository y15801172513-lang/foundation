...
本地 038 候选（尚未发布）增加获取期间只读进度页与连续项目交付检查；旧固定 npm 0.1.3 不含这些变化，不能仅更新运行包便宣称下载页已更新。正式同步前，下面稳定入口和发行身份保持不变。临时进度页只观察本次操作；页面失效后用原结果记录恢复，不自动重试或继承此前批准。
...
当前运行版 [0.2.9](https://github.com/y15801172513-lang/foundation/releases/tag/v0.2.9) 已发布并核验发行身份与资产；npm 薄入口未改，继续使用 0.1.3。运行载荷绑定公共源码 `920b9e6ef699f83aea09b061a423b8f06c6f42d3`，文档更新不改变该不可变身份。以下入口可用于新对话试用；真人安装、接续与主动回复仍独立验收：
...
旧 npm 0.1.1 / v0.2.6 只支持先由对话取得目录再 `--prepare` 的流程；裸命令止于 `RELEASE_DISCOVERED_NOT_ACQUIRED` 是旧版已知缺陷。新试用使用上面的 0.1.3 入口，不复用旧获取缓存或旧计划。现有安装不会因为新版发布自动更新。

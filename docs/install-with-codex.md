...
当前支持边界见[现行支持策略](install-platform-024.md#现行支持策略)。旧版退出维护；当前单页试用版为程序 0.2.31、npm 0.1.13。
...
当前不可变发行：[0.2.31](https://github.com/y15801172513-lang/foundation/releases/tag/v0.2.31)、[npm 0.1.13](https://www.npmjs.com/package/@josephyulei/summon-foundation/v/0.1.13)，运行源码 `9aaaf3c28498df9301cfb5993b77fe364d66b583`，与该发行证明绑定一致。文档提交不替代运行源码身份；旧资产不覆盖。
...
当前 0.2.31 / npm 0.1.13 已发布：当前用户共用一份 Foundation 和一份可选的 Codex 对话功能。安装只选软件位置，不选择业务项目；各项目以后按需接入，文件、规则和例外分别保管。启用后请在新 Codex 对话中尝试“打开 Foundation”或“用 Foundation 做这个项目”；文件就位不等于实际识别已通过。旧 0.2.13 / 0.1.7 的不可变发行保留，不被覆盖。

...
0.2.10 与 npm 0.1.4 配合提供获取期间只读进度页、连续生命周期总览与项目交付检查；旧固定 npm 0.1.3 不含进度页，不能仅更新运行包便宣称下载页已更新。临时进度页只观察本次操作；页面失效后用原结果记录恢复，不自动重试或继承此前批准。整体完成、能力材料准备、宿主注册与实际新任务发现分别判断。

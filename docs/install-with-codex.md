...
页面可选当前用户（推荐）或指定 Git 项目。项目模式绑定用户明确选择的真实仓库根及身份；程序用不与业务项目互相包含的独立目录，专用运行时随其确权。Skill 放在该仓库 `.agents/skills/ai-product-foundation-kit`，不写用户级目录。Codex 按任务目录向上到仓库根查找项目 Skill；同名 Skill 不保证项目优先，已有用户级同名 Skill 会阻断新的项目注册，保留原文件，由用户另行决定冲突处理。文件写入不证明新任务已发现。[Codex 官方发现规则](https://learn.chatgpt.com/docs/build-skills)

范围由同一确认计划和已安装签名记录保存，更新不迁移范围。项目模式仅在绑定项目及其子目录启动使用；项目移动、替换或身份不符停止，不猜新路径。项目制作接入仍需独立确认。卸载按本安装归属清理程序和已批准的 Skill，保留其他安装、项目代码、事实、原有 Node 和独立缓存回执。

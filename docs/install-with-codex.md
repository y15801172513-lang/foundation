...
0.2.10 与 npm 0.1.4 配合提供获取期间只读进度页、连续生命周期总览与项目交付检查。039 修正候选为 Foundation 0.2.11 / npm 0.1.5，尚未发布：它补齐选中 Skill 的程序接续、明确维护入口和待续恢复；不能把下方旧命令视为已经包含修复。临时页面失效后用原结果记录恢复，不自动重试或继承此前批准。整体完成、能力材料准备、宿主注册与实际新任务发现分别判断。
...
当前运行版 [0.2.10](https://github.com/y15801172513-lang/foundation/releases/tag/v0.2.10) 已发布并核验发行身份与资产；薄入口使用 npm 0.1.4。运行载荷绑定公共源码 `163972c56c94f8b89f09e72411d28c8540dc4f0c`，文档更新不改变该不可变身份。以下入口可用于新对话试用；真人安装、接续与主动回复仍独立验收：

```text
npx --yes --package @josephyulei/summon-foundation@0.1.4 summon foundation
```
...
旧 npm 0.1.1 / v0.2.6 只支持先由对话取得目录再 `--prepare` 的流程；裸命令止于 `RELEASE_DISCOVERED_NOT_ACQUIRED` 是旧版已知缺陷。新试用使用上面的 0.1.4 入口，不复用旧获取缓存或旧计划。现有安装不会因为新版发布自动更新。

...
Foundation 0.2.11 与 npm 0.1.5 提供获取进度、目录选择和连续生命周期总览。选中 Skill 后由程序接续独立确认；更新、卸载和待续恢复使用已安装入口。临时页面失效后读取同次结果，不自动重试或继承此前批准。整体完成、材料准备、注册与实际新任务发现分别判断。
...
当前运行版 [0.2.11](https://github.com/y15801172513-lang/foundation/releases/tag/v0.2.11) 已发布并核验发行身份与资产；薄入口使用 npm 0.1.5。运行载荷绑定公共源码 `ede0dd48296f7498f77d733996c663f55663721c`，文档更新不改变该不可变身份。以下入口可用于新对话试用；真人安装、接续与主动回复仍独立验收：

```text
npx --yes --package @josephyulei/summon-foundation@0.1.5 summon foundation
```
...
旧 npm 0.1.1 / v0.2.6 只支持先由对话取得目录再 `--prepare` 的流程；裸命令止于 `RELEASE_DISCOVERED_NOT_ACQUIRED` 是旧版已知缺陷。新试用使用上面的 0.1.5 入口，不复用旧获取缓存或旧计划。现有安装不会因为新版发布自动更新。

...
本轮目标发行（发布状态以线上验证为准）：[0.2.17](https://github.com/y15801172513-lang/foundation/releases/tag/v0.2.17)、[npm 0.1.12](https://www.npmjs.com/package/@josephyulei/summon-foundation/v/0.1.12)，运行源码身份以该发行证明绑定为准。文档提交不替代运行源码身份；旧资产不覆盖。
...
本轮 0.2.17 / npm 0.1.12 发布目标：当前用户共用一份 Foundation 和一份可选的 Codex 对话功能。安装只选软件位置，不选择业务项目；各项目以后按需接入，文件、规则和例外分别保管。启用后请在新 Codex 对话中尝试“打开 Foundation”或“用 Foundation 做这个项目”；文件就位不等于实际识别已通过。旧 0.2.13 / 0.1.7 的不可变发行保留，不被覆盖。
...
不支持安全原地更新时，恢复方案是另行说明并获得卸载及新版首装授权，经各自精确确认执行；新版首装使用另行选定且检查通过的新目录，不覆盖旧目录留下的资料。保留项目资料和未知/修改文件，Skill 解除与重新接续分别确认。恢复首装必须选择已发布且包含对应修复的版本；重装仍受相同路径限制的旧正式包不算解决。已有项目先独立确认停用并保留事实，再由新安装显式接入，不能把目录搬移或旧机器注册当作新授权。不得直接修改旧安装或把未来源码修复说成旧引擎已具备的能力。未发布修复包前，源码测试通过不代表现有安装已恢复更新。

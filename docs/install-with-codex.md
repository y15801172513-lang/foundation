...
正在本地验证的简化版（尚未发布）：当前用户共用一份 Foundation 和一份可选的 Codex 对话功能。安装只选软件位置，不选择业务项目；各项目以后按需接入，文件、规则和例外分别保管。启用后请在新 Codex 对话中尝试“打开 Foundation”或“用 Foundation 做这个项目”；文件就位不等于实际识别已通过。上方 0.2.13 / 0.1.7 仍是原不可变发行，不会因本文修订而改变。

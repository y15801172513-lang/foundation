...
function TokenSample({asset,assets}) {
  ...
  const valid=token.type==='font-family'||CSS.supports(token.type==='color'?'color':'width',token.value);
  if(!valid)return <PreviewMessage status="error" title="变量值不可用" description="当前浏览器无法解析此值，请核对来源。"/>;
  return <section className="flex flex-col gap-4 p-4" data-token-type={token.type}><SectionTitle>设计变量样例</SectionTitle>{token.type==='color'?<div className="size-16 rounded-md border" style={{backgroundColor:token.value}} aria-label={'色样 '+token.value}/>:token.type==='spacing'?<div className="h-4 bg-primary" style={{width:token.value}} aria-label={'间距 '+token.value}/>:<p className="text-base leading-relaxed" style={{fontFamily:token.value}}>产品设计 Aa 0123</p>}<MetadataText>{token.value} · {token.source}</MetadataText>{token.type==='font-family'?<MetadataText role="status">{fontState}</MetadataText>:null}<ContentDescription>样例展示登记值；页面主题与继承环境需另行核验。</ContentDescription></section>;
}

import {DeliveryAssessment} from '@/components/foundation/delivery-assessment';
...
export function InformationLogicWorkspace({model, pageId, theme = 'light', onSelectPage, onOpenPage, onRelationSaved, onRelationsRefreshed = () => {}}) {
  const projection = useMemo(() => logicFlowModel({pages: model.pages, relations: model.relations, assets: model.assets, interactions: model.interactions, changes: model.changes, figma: model.figma, delivery:model.delivery, currentPageId: pageId}), [model.pages, model.relations, model.assets, model.interactions, model.changes, model.figma, model.delivery, pageId]);
  ...
  const [editor, setEditor] = useState(null); const [saving, setSaving] = useState(false); const [error, setError] = useState(''); const [conflict, setConflict] = useState(null); const [selectedRelationId, setSelectedRelationId] = useState(null); const [notice, setNotice] = useState('');
  useEffect(() => {
    ...
    setEdges((current) => [...projection.edges, ...current.filter((edge) => edge.id === 'relation-draft')]);
  }, [projection, theme]);

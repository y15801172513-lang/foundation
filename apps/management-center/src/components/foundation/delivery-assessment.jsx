...
export function DeliveryAssessment({assessment}) {
  return <section className="flex flex-col gap-2 py-3" data-delivery-assessment={assessment?.aggregate || 'pending'}><SectionTitle>交付验收 · {states[assessment?.aggregate] || '待核'}</SectionTitle>{assessment?.task && <MetadataText>当前任务：{states[assessment.task.state] || '待核'} · 项目整体：{states[assessment.project?.state] || '待核'}</MetadataText>}<ContentDescription>{Object.entries(labels).map(([key,label])=>`${label}：${states[assessment?.[key]?.state] || '待核'}`).join(' · ')}</ContentDescription>{assessment?.issues?.slice(0,3).map(issue=><MetadataText key={issue.id || issue.message}>{issue.message}</MetadataText>)}<MetadataText>内容一致和资料已更新不等于验收通过。</MetadataText></section>;
}

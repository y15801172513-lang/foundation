...
export async function observeWorkbenchCapabilities({devtools,devtoolsPort,sourceSceneDigest,workbenchUrl,projectId,revision,pageId,assetUrl}) {
  ...
  if(assetUrl) {
    const actual=await evaluate(devtools,`new Promise(resolve=>{const f=document.createElement('iframe'),url=new URL(${JSON.stringify(assetUrl)}),q=url.searchParams;f.sandbox='allow-scripts';f.style.cssText='position:fixed;inset:0;width:100vw;height:100vh;z-index:2147483647;border:0;background:white';f.dataset.foundationVerifierAsset='';let timer;const observed=[];const finish=value=>{clearTimeout(timer);removeEventListener('message',receive);resolve(value)};const receive=event=>{const d=event.data;if(event.source===f.contentWindow&&observed.length<10)observed.push({origin:event.origin,namespace:String(d?.namespace||'').slice(0,64),kind:String(d?.kind||'').slice(0,64),status:String(d?.status||'').slice(0,64),identityMatches:['projectId','revision','channel','assetId'].every(k=>d?.[k]===q.get(k))});if(event.source!==f.contentWindow||event.origin!=='null'||d?.namespace!=='ai-product-foundation-asset-preview'||d.kind!=='status'||['projectId','revision','channel','assetId'].some(k=>d[k]!==q.get(k)))return;if(['ready','error','unsupported'].includes(d.status))finish({status:d.status,projectId:d.projectId,revision:d.revision,assetId:d.assetId,channel:d.channel,sandbox:f.getAttribute('sandbox')});};addEventListener('message',receive);f.addEventListener('load',()=>f.contentWindow.postMessage({namespace:'ai-product-foundation-asset-preview',kind:'status-request',...Object.fromEntries(['projectId','revision','channel','assetId'].map(k=>[k,q.get(k)]))},'*'));timer=setTimeout(()=>finish({status:'timeout',observed}),4000);f.src=url.href;document.body.append(f);})`);
    let rendered=null;

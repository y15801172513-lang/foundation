...
export function PreviewSidebar({view = 'page', model, pageId, selectedEvent = null, inspectedObject = null, onNavigate, onPreview, onCopyTask, expandedInspectorNodes, onExpandedInspectorNodesChange, inspectorTreeFocusId, onInspectorTreeFocusIdChange, treeActive = false}) {
  ...
  return <section className="preview-sidebar-content" data-sidebar-view="page"><header className="preview-sidebar-heading"><MetadataText as="p" className="sidebar-kicker">当前页面</MetadataText><PanelTitle>{sidebar.page.name || '尚未登记'}</PanelTitle><ContentDescription><CodeText>{sidebar.page.id}</CodeText> · {sidebar.page.preview || sidebar.page.route || 'route 尚未登记'}</ContentDescription></header><DeliveryAssessment assessment={sidebar.deliveryAssessment}/><ContentDescription>{sidebar.entryLabel}</ContentDescription><>{sidebar.page.navigationApplicability?.state === 'not-applicable' ? <ContentDescription>跨页导航不适用：{sidebar.page.navigationApplicability.reason}</ContentDescription> : <><RelationGroup title="从哪里进入" relations={sidebar.incoming} direction="incoming" /><RelationGroup title="可以去哪里" relations={sidebar.outgoing} direction="outgoing" /></>}</><section className="preview-sidebar-section"><SectionTitle>页面内状态变化</SectionTitle>{sidebar.stateChanges.length ? <ul>{sidebar.stateChanges.map((item) => <li key={item.id}><strong>{item.name || item.id}</strong><span>{item.description || '状态变化说明尚未登记'}</span></li>)}</ul> : <ContentDescription>{sidebar.page.statesApplicability?.state === 'not-applicable' ? `不适用：${sidebar.page.statesApplicability.reason}` : '尚未登记页面内状态变化。'}</ContentDescription>}</section><RuntimeEventState selectedEvent={selectedEvent} />{sidebar.gaps.length ? <section className="preview-sidebar-section"><SectionTitle>需要处理</SectionTitle><ul>{sidebar.gaps.map((gap) => <li key={gap}>{gap}</li>)}</ul></section> : null}</section>;
}

...
function HumanSection({section}) {
  const entries = section.entries || section.items.map((text, index) => ({id: `${section.id}:${index}`, text}));
  const content = <div className="human-section-content"><SectionTitle>{section.title}</SectionTitle><ul>{entries.map((entry) => <li key={entry.id} data-context-entry={entry.id}>{entry.text}</li>)}</ul></div>;
  return <section data-context-section={section.id} className={section.id === 'technical' ? 'human-section human-technical' : 'human-section'}>{content}</section>;
}

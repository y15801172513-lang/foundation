...
039 接续修正正在本地验证（Foundation 0.2.11 / summon 0.1.5，尚未发布）。它补齐所选 Skill、更新与卸载的程序接续；下方已发布命令不会因源码改变而自动获得修复。
...
在一个**不属于源码建设目录、允许写入安装位置的新 Codex 对话**里粘贴以下命令。运行版 0.2.10 已发布并完成发行字节核验，npm 0.1.4 提供获取进度页和连续操作总览；新对话安装体验仍待真人验收：
...
当前运行版为 [v0.2.10](https://github.com/y15801172513-lang/foundation/releases/tag/v0.2.10)。新版入口会实际下载并准备目录选择页，不再只打印接续提示；但无历史新 Codex 对话是否正确打开页面、持续等待并主动告知结果，仍须真人验收。默认使用 Codex 内置浏览器，系统浏览器仅在告知并同意后备用。当前版本与过渡方法见[安装说明](docs/install-with-codex.md)。

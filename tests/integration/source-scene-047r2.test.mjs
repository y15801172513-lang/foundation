...
test('047R2 candidate runtime renders source-bound export and fixed instances; forged configuration, identity and missing bridge fail',{timeout:60000},async t=>{
 ...
 fs.writeFileSync(path.join(project,'Counter.jsx'),`import {useState} from 'react';export function Counter({instanceId='计数器定义',tone='cool'}){const [count,setCount]=useState(0);return <section data-instance={instanceId} data-tone={tone}><h2>{instanceId}</h2><button onClick={()=>setCount(count+1)}>增加</button><output>{count}</output></section>}`);
 const d=analyzeSourcesInWorker({project,entryRoots:['Counter.jsx']}).definitions.find(d=>d.name==='Counter');
 const asset={id:'counter',implementationMapping:'Counter.jsx',assetModel:{variantAxes:[{key:'tone',values:['cool','warm']}],binding:{file:d.file,export:'Counter',declarationKind:d.declarationKind,anchor:d.anchor},configuration:[{key:'instanceId',kind:'string'}]},usageLocations:['item-a','item-b'].map(instanceId=>({instanceId,bindingId:instanceId,configuration:{instanceId}}))},facts={components:{items:[asset]}};
 ...
 const observations=[];
 for(const [testMode,instance]of [['normal',''],['normal','item-a'],['normal','item-b'],['variant','item-a'],['invalid','item-a'],['wrong','item-a'],['broken','item-a'],['fake','item-a']]) {
  mode=testMode;active=scenes.get(instance);const query=new URLSearchParams({mode,instanceId:instance,projectId:'p',revision:'r',channel:'c',assetId:mode==='wrong'?'other':'counter',scenarioId:'default',variantValues:mode==='fake'?'{"forged":true}':mode==='variant'?'{"tone":"warm"}':mode==='invalid'?'{"tone":"invented"}':'{}'});
  ...
  if(mode==='normal'||mode==='variant') {
   ...
   assert.equal(observation.tone,mode==='variant'?'warm':'cool');assert.equal(observation.instance,instance);assert.equal(observation.label,instance || '计数器定义');assert.equal(observation.sourceSceneDigest,active.renderDigest);assert.equal(observation.roots,1);assert.equal(observation.sections,1);assert(observation.width>0&&observation.height>0);assert.equal(observation.count,'1');assert.equal(observation.storageBlocked,true);observations.push({mode,instance,...observation});

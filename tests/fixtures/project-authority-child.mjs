...
const [{applyProjectAuthorityPlan}, {configureRuntimeControl}] = await Promise.all([
  import('../../packages/core/project-authority.mjs'),
  import('./test-runtime-surface.mjs'),
]);

const [planFile, pauseStage, markerFile] = process.argv.slice(2);
const plan = JSON.parse(fs.readFileSync(path.resolve(planFile), 'utf8'));
try {
  configureRuntimeControl({pauseAt: pauseStage, markerFile: path.resolve(markerFile)});
  const result = applyProjectAuthorityPlan({plan, now: plan.createdAt + 1});

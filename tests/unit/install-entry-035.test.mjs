...
test('035 full CLI no-TTY routes default to acquisition and held child, inspect never acquires',{skip:process.platform!=='darwin'||process.arch!=='arm64'},async()=>{
  ...
  for(const state of ['completed','failed','cancelled','expired','unexpected']){
    ...
    const x=await invoke('runtime-'+state,['foundation'],{FIXTURE_EVENT:JSON.stringify(event),FIXTURE_EXIT:state==='failed'?'1':'0'});assert.equal(x.r.status,state==='failed'?1:0,x.r.stderr);

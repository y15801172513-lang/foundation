...
test('035 full CLI no-TTY routes default to acquisition and held child, inspect never acquires',{skip:process.platform!=='darwin'||process.arch!=='arm64'},async()=>{
  ...
  fs.writeFileSync(work+'/child.mjs',`import fs from'node:fs';import{spawn as real}from'node:child_process';export function spawnSync(c,a){if(c==='/usr/bin/id')return{status:0,stdout:'fixture\\n'};if(c==='/usr/bin/dscl')return{status:0,stdout:'NFSHomeDirectory: '+process.env.PROBE_HOME+'\\n'};if(a[0]==='--help')return{status:0,stdout:'--choose-destination --journey-channel'};throw Error('unexpected spawnSync');}export function spawn(c,a,o){fs.writeFileSync(process.env.PROBE_HOME+'/spawn.json',JSON.stringify({c,a,simulation:true}));return real(process.execPath,['-e',"console.log('TEST_CHILD_STARTED');setTimeout(()=>console.log('TEST_CHILD_ENDED_NO_INSTALL'),30)"],o);}`);
  fs.writeFileSync(work+'/loader.mjs',`export async function resolve(s,c,n){if(c.parentURL===${JSON.stringify(entry)}||c.parentURL?.endsWith('/lib/acquisition-worker.mjs')){const map={'../lib/acquire.mjs':'acquire.mjs','./acquire.mjs':'acquire.mjs','node:child_process':'child.mjs',...(process.env.SOURCE_GUARD_TEST?{}:{'node:fs':'fs.mjs'})};if(map[s])return{url:new URL(map[s],import.meta.url).href,shortCircuit:true};}return n(s,c);}`);
  fs.writeFileSync(work+'/register.mjs',`import{register}from'node:module';register(new URL('./loader.mjs',import.meta.url));`);
  const results=[];
  const invoke=async(name,args,extra={})=>{const home=work+'/'+name;fs.mkdirSync(home);let stdout='',stderr='',selected=false,ack=false;const child=spawn(process.execPath,['--import',work+'/register.mjs',new URL(entry).pathname,...args],{cwd:repo,env:{...process.env,NODE_OPTIONS:'',PATH:'',TMPDIR:work,PROBE_HOME:home,...extra}});const timer=setTimeout(()=>child.kill('SIGTERM'),10000);
    child.stdout.on('data',bytes=>{stdout+=bytes;const events=stdout.split('\n').filter(l=>l.startsWith('{"status":"FOUNDATION_INSTALL_OPERATION"')).flatMap(l=>{try{return[JSON.parse(l)]}catch{return[]}});const latest=events.at(-1);if(latest?.phase==='choosing-intent'&&!selected){selected=true;fetch(latest.progressUrl+'/action',{method:'POST',headers:{origin:new URL(latest.progressUrl).origin,'content-type':'application/json'},body:JSON.stringify({csrf:latest.progressUrl.split('/').at(-1),operationId:latest.operationId,action:'choose',destination:home+'/Foundation',skillChoice:'skipped'})}).catch(e=>stderr+=e.message);}if(latest?.terminal&&latest.progressUrl&&!ack){ack=true;fetch(latest.progressUrl+'/state').catch(()=>{});}});
    child.stderr.on('data',b=>stderr+=b);const status=await new Promise(resolve=>child.once('close',resolve));clearTimeout(timer);const r={status,stdout,stderr};results.push({name,args,...r});fs.writeFileSync(work+'/results.json',JSON.stringify(results,null,2));return{r,home};};
  const inspect=await invoke('inspect',['foundation','--inspect']);assert.equal(inspect.r.status,0,inspect.r.stderr);assert.equal(JSON.parse(inspect.r.stdout).status,'RELEASE_DISCOVERED_NOT_ACQUIRED');assert.deepEqual(fs.readdirSync(inspect.home),[]);
  const normal=await invoke('default',['foundation']);assert.equal(normal.r.status,1,normal.r.stderr);const unconfirmed=normal.r.stdout.split('\n').filter(l=>l.startsWith('{"status":"FOUNDATION_INSTALL_OPERATION"')).map(l=>JSON.parse(l)).at(-1);assert.equal(unconfirmed.state,'verification-required');assert.equal(unconfirmed.installationWrites,'unknown');assert.match(normal.r.stdout,/TEST_CHILD_STARTED/);assert.match(normal.r.stdout,/TEST_CHILD_ENDED_NO_INSTALL/);assert(!normal.r.stdout.includes('RELEASE_DISCOVERED_NOT_ACQUIRED'));const spawned=JSON.parse(fs.readFileSync(normal.home+'/spawn.json')).a;assert.equal(spawned[0],'install');assert.equal(spawned[1],'--journey-id');assert.match(spawned[2],/^[a-f0-9-]+$/);assert.deepEqual(spawned.slice(3),['--choose-destination','--journey-channel','--browser','codex']);
  const network=await invoke('network',['foundation'],{FAIL_NETWORK:'1'});assert.equal(network.r.status,1);const early=network.r.stdout.split('\n').filter(l=>l.startsWith('{"status":"FOUNDATION_INSTALL_OPERATION"')).map(l=>JSON.parse(l)).at(-1);assert.equal(early.phase,'discovering');assert.equal(early.state,'failed');assert(early.resultFile);assert.equal(JSON.parse(fs.readFileSync(early.resultFile)).installationWrites,'none');
  const guard=await invoke('guard',['foundation'],{SOURCE_GUARD_TEST:'1'});assert.equal(guard.r.status,1);assert.match(guard.r.stderr,/SOURCE_ONLY/);assert.deepEqual(fs.readdirSync(guard.home),[]);
  const failed=await invoke('acquisition-failed',['foundation'],{FAIL_ACQUISITION:'1'});assert.equal(failed.r.status,1);assert(!failed.r.stderr.includes('secret'));assert(!failed.r.stderr.includes('pass@'));
  const events=failed.r.stdout.split('\n').filter(l=>l.startsWith('{')).map(l=>JSON.parse(l)).filter(e=>e.status==='FOUNDATION_INSTALL_OPERATION');assert(events.length>=3);assert.equal(new Set(events.map(e=>e.operationId)).size,1);const last=events.at(-1);assert.equal(last.state,'failed');assert.equal(last.phase,'fetching-and-verifying-runtime');assert.equal(last.installationWrites,'none');assert.equal(last.diagnostic.transport.curlExitCode,28);assert(last.resultFile);assert.equal(JSON.parse(fs.readFileSync(last.resultFile)).state,'failed');
  const restored=await invoke('restored',['foundation','--status',last.resultFile]);assert.equal(restored.r.status,0);assert.equal(JSON.parse(restored.r.stdout).operationId,last.operationId);assert.deepEqual(fs.readdirSync(restored.home),[]);
  ...
  for(const state of ['completed','completed-with-proof','failed','cancelled','expired','unexpected']){
    const event={status:'BOOTSTRAP_OPERATION_ENDED',state:state==='completed-with-proof'?'completed':state,sessionId:'fixture-session',installationRoot:'/fixture/中文 空格',result:{...(state==='completed-with-proof'?{ok:true}:{}),executableHealth:state==='failed'?'failed':'unknown'}};
    const x=await invoke('runtime-'+state,['foundation'],{FIXTURE_EVENT:JSON.stringify(event),FIXTURE_EXIT:state==='failed'?'1':'0'});assert.equal(x.r.status,state==='completed-with-proof'?0:state==='cancelled'?2:state==='expired'?3:1,x.r.stderr);
    const record=x.r.stdout.split('\n').filter(l=>l.startsWith('{"status":"FOUNDATION_INSTALL_OPERATION"')).map(l=>JSON.parse(l)).at(-1);assert.equal(record.state,state==='completed-with-proof'?'completed':['completed','unexpected'].includes(state)?'verification-required':state);assert.equal(record.installationRoot,event.installationRoot);assert.equal(record.runtimeHealth,event.result.executableHealth);assert.equal(readonly(record.resultFile).operationId,record.operationId);
  }

...
// Fresh installs are shared by the current user. Reject stale project intent;
// never silently reinterpret it. Existing maintenance keeps its signed scope.
function scopeFields(value){
  inspectScopeIntent(value.scopeKind===undefined?'user':value.scopeKind,value.projectRoot);
  return {};
}
  ...
  return {
    ...
    async submit(body){
      ...
      if(body.action==='choose'&&resolveChoice){
        ...
        choice={destination:destinationCheck.destination,skillChoice:body.skillChoice,scopeKind:'user',projectRoot:null,destinationCheck};const resolve=resolveChoice;clearTimeout(choiceTimer);resolveChoice=null;rejectChoice=null;resolve(choice);changed();return {status:200,body:{state:'intent-selected'}};
      }

...
本地 0.1.4 候选新增获取期间只读进度页，尚未发布；以下已发布命令保持到正式同步。获取成功后同一页面关联运行程序返回的目录/确认页，仍由 Codex 内置浏览器打开。临时服务只随本次任务运行，不是守护进程，不执行未核验归档，也不替本人确认。原操作回执仍可独立只读查询，页断开不改写已核实终态。
...
0.1.3 入口配合 Foundation 0.2.8 或后续支持目录选择的正式发行使用。默认命令实际查询、固定版本、披露缓存、下载并校验，然后保持运行等待安装页目录选择。选择目录后还要亲自确认精确安装计划；不会注册 Skill 或接入项目。实际获取仍须通过 registry 与 Release 校验，本文不替代发布证明。

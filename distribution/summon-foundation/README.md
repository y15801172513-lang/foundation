...
本地 036 候选 0.1.3 尚未发布：联网前建立同次操作记录，输出阶段、大文件实际字节/总量、尝试次数与最近进展；不能保存早期记录则不开始联网。大文件每次最多 10 分钟，连接 15 秒、低速 60 秒有界停止，仅暂时网络错误可重试一次。失败保留 curl 退出码和脱敏类别，校验失败与网络失败分开。`--status <返回的结果文件>` 只读恢复，不获取、安装或重试；中间记录不证明进程仍在运行。新运行版安装后程序核验稳定入口，启动唯一工作台至就绪便返回结果，不等常驻工作台退出；Codex 打开返回的实际网址并解释结果。已发布 0.1.2 不含上述修复。工具返回 running 不是任务完成，运行中的 Codex 应继续观察原句柄；程序无法强制宿主不提前结束，新对话无追问反馈仍待验证。

...
0.1.10 单页试用入口与 Foundation 0.2.16 的运行引擎共同支持当前用户共用安装及清晰的完成、取消和部分完成反馈。同一页面内保留目录选择、程序和 Skill 独立确认。真实安装、新任务发现和主动告知仍需独立验收。
...
```text
npx --yes --package @josephyulei/summon-foundation@0.1.10 summon foundation
```

...
0.1.6 单页试用入口与 Foundation 0.2.12 起的运行引擎共同支持同一页面内的目录选择、程序和 Skill 独立确认；旧发行版本不具备本次单页修订。真实安装、新任务发现和主动告知仍需独立验收。
...
```text
npx --yes --package @josephyulei/summon-foundation@0.1.6 summon foundation
```
